...
// startups qui n'ont jamais aucun signal presse détecté (voir échange du
// 2026-07-24 : sur 42 startups, seules ~10 apparaissent dans un snapshot de
// nos 3 flux — la tendance/le buzz ci-dessus resteraient à 0 indéfiniment
// pour la majorité du roster). Contrairement à la tendance/au buzz
// (réactions à un signal réel détecté), ce bruit est délibérément
// synthétique : chaque startup a un régime persistant (croissance /
// stagnation / décroissance stocké dans startups.momentum_regime — voir
// supabase/018_market_noise.sql), qui change rarement, et qui biaise un
// petit pas quotidien. Comme le reste de valuation_offset_pct, ça ne touche
// jamais l'ANCRE (dernière levée réelle sourcée) — uniquement l'oscillation
// synthétique autour, qui a toujours été un mécanisme de jeu.
//
// Déterministe (seed = startup_id + date), pas Math.random() brut : un
// rejeu du cron le même jour (retry Vercel...) doit retomber sur exactement
// le même résultat plutôt que de re-tirer un nouveau pas à chaque appel.
const REGIME_TRANSITION_PROB = 0.08; // ~8%/jour de chance de changer de régime
...
// Décide si le régime change aujourd'hui (et vers lequel) — jamais un
// "changement" vers le même régime qu'avant.
export function nextMomentumRegime({ startupId, dateStr, currentRegime }) {
  const rng = mulberry32(hashSeed(`${startupId}|${dateStr}|regime`));
  if (rng() < REGIME_TRANSITION_PROB) {
    const others = REGIMES.filter((r) => r !== currentRegime);
