...
//    - trends  : tendance de recherche (Google Trends)
...
// les mêmes proportions entre eux (funding 50%, trends 25%, press 25%).
...
export function computeMomentumScore({ funding, trends, press, github }) {
  const universalScore =
    funding * UNIVERSAL_WEIGHTS.funding +
    trends * UNIVERSAL_WEIGHTS.trends +
  ...
  if (!isGithubApplicable) {
    const universalWeightSum =
      UNIVERSAL_WEIGHTS.funding + UNIVERSAL_WEIGHTS.trends + UNIVERSAL_WEIGHTS.press;
